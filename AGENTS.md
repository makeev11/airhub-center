...
## Внутренняя база знаний платформы

Для вопросов об устройстве Center, HQ, Site и агентов начните с соседней
внутренней базы: [`airhop-knowledge/AGENTS.md`](../airhop-knowledge/AGENTS.md)
и [`INDEX.md`](../airhop-knowledge/INDEX.md). Она содержит короткие объяснения,
первичные источники и проверку их изменений. Это отдельный локальный/private
vault, не родительская база знаний центров и не часть публичной поставки.
Если папки нет, используйте канонические документы этого repo.
После изменения поведения или закрытия инцидента обновляйте связанные статьи
и evidence в рамках задачи. Права на действия и правила выкладки по-прежнему
определяются инструкциями соответствующего продукта и канонической картой ниже.

Для новой фичи, существенного изменения поведения, подготовки к разработке
или закрытия release-задачи используйте `$airhop-task-cycle` из
`.agents/skills/airhop-task-cycle/SKILL.md`. Канон навыка находится в соседнем
`airhop-knowledge`; локальное подключение: `python3 tools/connect_projects.py
--apply` из vault. Продолжайте уже начатую постановку, не запускайте второй
опрос и не спрашивайте известное. Для вопроса, ревью или опечатки полный
intake не нужен. Если vault недоступен, явно сохраните handoff в текущей задаче
и сообщите о непереданных знаниях; не придумывайте расположение другого repo.
Внутреннее описание обновляется в `airhop-knowledge`, клиентские статьи — в
`airhop-knowledge-cl`. Подготовка статьи не разрешает публичную публикацию:
нужны проверка фактического релиза и отдельное подтверждение пакета.
