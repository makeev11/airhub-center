...
> **Историческая запись, частично заменена.** Решение сохранить нативный
> `input[type=date]` оказалось регрессией в Safari: браузер вернул неудобный
> системный календарь вместо согласованного AirHop-поля. А обработчик Enter
> внутри React-дерева не получал нажатие после pointer-click в WebKit, когда
> фокус оставался на `body`. Актуальный контракт и исправляющий production-релиз
> зафиксированы в
...
В этом историческом кандидате нативное поле даты было сохранено как
`input[type=date]`: public booking задавал ему
...
не заменяло их собственной маской. Это решение больше не является действующим:
исправляющий релиз вернул `AirHopDateInput`. Одновременно была устранена утечка
