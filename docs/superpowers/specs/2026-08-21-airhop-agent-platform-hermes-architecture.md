...
## Реализованный срез общей платформы — 2026-09-11

Контракт текущего среза уточнён в `docs/AIRHOP_SOURCE_OF_TRUTH.md`, раздел
«Общая платформа команды и инициативная работа». Реализация и результаты проверки:
`docs/AIRHOP_AGENT_PLATFORM_IMPLEMENTATION_20260911.md`.

Добавлены общий ExecutionGraph для пяти продуктовых ролей, серверные политики
9052, закрытые процедурные команды 9053, UI обязанностей, серверные birthday/report
duties и атомарные квитанции внутренних ответов. Runtime внутренних built-in ролей
с незаданным harness теперь выбирает закреплённый Airhop Hermes profile. Profile
изоляция учитывает центр и ключ агента; native shared memory и посторонние инструменты
отключены. Установка/проверка описана в `integrations/hermes-airhop-team-runtime`.

Это не завершение всех целевых слоёв документа: graph database/universal graph
search, hosted reconciler для всех внутренних ролей и полноценный learned-skill
quality pipeline не реализованы. Текущий вариант обучения сохраняет только категории
успешных чтений после принятого ответа, проверяет role/graph compatibility и допускает
ручной выбор/откат после трёх независимых наблюдений. Наблюдения не доказывают улучшение
качества, конверсии или стоимости. Автоматическая активация без решения администратора
не включена.
