...
mod tests {
    ...
    fn embedded_migrator_contains_consolidated_initial_schema() {
        ...
        assert!(migrations[70]
            .sql
            .as_str()
            .contains("CREATE TABLE airhop_human_takeover_reminders"));
        assert!(migrations[70]
            .sql
            .as_str()
            .contains("human_staff_outbound_count"));
        assert_eq!(migrations[69].version, 70);
        assert!(migrations[69]
            .sql
            .as_str()
            .contains("CREATE TABLE airhop_whatsapp_delivery_receipts"));
        ...
        let airhop_payments = migrations[35].sql.as_str();
        assert!(airhop_payments.contains("CREATE TABLE airhop_payment_expectations"));
        assert!(airhop_payments.contains("tariff_name_snapshot"));

        assert_eq!(migrations[36].version, 37);
        let airhop_payment_automation = migrations[36].sql.as_str();
        assert!(airhop_payment_automation.contains("payments_buzz_channel_id"));
        assert!(
            airhop_payment_automation.contains("CREATE TABLE airhop_payment_buzz_summary_state")
        );
        assert!(airhop_payment_automation.contains("ADD COLUMN billing_period"));
        assert!(airhop_payment_automation.contains("enrollment_period_key"));
        assert!(airhop_payment_automation.contains("pending_digest"));

        assert_eq!(migrations[37].version, 38);
        let airhop_payment_confirmation = migrations[37].sql.as_str();
        assert!(airhop_payment_confirmation.contains("ADD COLUMN pending_actions"));
        assert!(
            airhop_payment_confirmation.contains("CREATE TABLE airhop_payment_buzz_action_state")
        );
        assert!(airhop_payment_confirmation.contains("payment_version"));
        assert!(airhop_payment_confirmation.contains("event_id"));

        assert_eq!(migrations[38].version, 39);
        let airhop_enrollment_lifecycle = migrations[38].sql.as_str();
