    ...
    pub async fn resolve_airhop_staff_control_batch(
        ...
        let row = sqlx::query(
            "SELECT conversation.organization_id, conversation.id, conversation.channel_id,
                    conversation.family_id, conversation.representative_id, conversation.parent_pubkey,
                    conversation.current_cycle_id, conversation.owner, conversation.hermes_paused,
                    conversation.control_version, conversation.created_at, conversation.updated_at,
                    conversation.threaded, encode(conversation.root_event_id,'hex') AS root_event_id,
                    message.id AS command_id, message.content, message.pubkey, message.kind,
             ...
             WHERE NOT EXISTS (
               ...
               AND NOT EXISTS (SELECT 1 FROM events newer
                 ...
                 WHERE newer.community_id=message.community_id AND newer.channel_id=message.channel_id
                  ...
                  AND (NOT conversation.threaded OR newer.id=conversation.root_event_id OR EXISTS (
                    SELECT 1 FROM thread_metadata thread WHERE thread.community_id=newer.community_id
                     AND thread.event_id=newer.id
