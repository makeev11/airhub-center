...
def backup_and_preflight():
    ...
    backup = ROOT / 'backup'
    require(backup.is_dir(), 'Original guarded backup is missing')
    original = json.loads((ROOT / 'backup-receipt.json').read_text())
    require(sha(Path(original['path'])) == original['sha256'], 'Original backup hash differs')
    dump = backup / 'buzz-v2.dump'
    ...
    record('backup-receipt-v2.json', {'path': str(dump), 'sha256': sha(dump),
    ...
    try:
        ...
        run('docker', 'run', '-d', '--name', PREFLIGHT_CONTAINER,
            ...
            '-e', 'BUZZ_GIT_REPO_PATH=/tmp/repos',
        ...
        record('migration-preflight-receipt-v2.json', {'backup_restored': True, 'schema': 71,
...
def apply():
    ...
    runner = json.loads((ROOT / 'runner-v2-receipt.json').read_text())
    require(Path(__file__).resolve() == ROOT / 'release-v2.py' and
            sha(Path(__file__)) == runner['sha256'], 'Reviewed v2 runner hash differs')
...
def main():
    ...
    with open('/opt/airhop/buzz-demo/deploy.lock', 'a') as lock:
        ...
        elif action == 'install-runner-v2':
            predecessor_guard()
            sealed_guard()
            destination = ROOT / 'release-v2.py'
            with destination.open('xb') as output:
                output.write(Path(__file__).read_bytes())
            os.chmod(destination, 0o555)
            record('runner-v2-receipt.json', {'sha256': sha(destination),
                   'reason': 'Isolated read-only preflight requires BUZZ_GIT_REPO_PATH=/tmp/repos; original guard and image inputs unchanged',
                   'original_preflight_failed_before_db_connection': True,
                   'original_binary_and_plan_unchanged': True, 'installed_at': time.time()})
        elif action == 'apply':
            apply()
        else:
