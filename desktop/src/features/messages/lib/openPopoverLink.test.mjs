...
test("legacy buzz://message deep-link still routes in-app", () => {
  const { handlers, external, inApp } = makeSpies();
...
test("non-message app URLs fall through to the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  openPopoverLink("airhop://channel?foo=bar", handlers);
  assert.deepEqual(external, ["airhop://channel?foo=bar"]);
  assert.equal(inApp.length, 0);
});

test("malformed airhop://message URL falls back to the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  // Matches isMessageLink (starts with airhop://message?) but is missing the
  // required channel/id params, so parse fails and we don't navigate in-app.
  openPopoverLink("airhop://message?nope=1", handlers);
  assert.deepEqual(external, ["airhop://message?nope=1"]);
  assert.equal(inApp.length, 0);
});
