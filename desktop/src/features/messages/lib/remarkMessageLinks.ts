/**
 * Remark plugin that detects bare `airhop://message?…` URLs in text nodes and
 * replaces each with a custom `message-link` HAST element. Legacy
 * `buzz://message?…` URLs remain accepted. The `markdown.tsx`
 ...
 * and `www.`. Custom schemes like `airhop://` only reach the `<a>` component
 * override when the user wrote an explicit `[label](airhop://…)` link.
 *
...
const MESSAGE_URL_PATTERN = /(?:airhop|buzz):\/\/message\?[^\s<>"')\]]+/g;
