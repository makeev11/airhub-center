...
export const bookingSchema = z.object({
  ...
  source: z.object({
    surface: z.enum(["standalone", "embedded", "staff_ui", "buzz_agent"]),
...
const enrollmentBaseShape = {
  ...
  source: z.enum(["staff_ui", "buzz_agent", "import"]),
  createdBy: z.string().trim().min(1).max(200),
  createdAt: z.string().datetime({ offset: true }),
  updatedAt: z.string().datetime({ offset: true }),
};
...
export const pendingActionSchema = z.object({
  ...
  preparedByAgentId: z.string().trim().min(1).max(200),
  specialistRole: z.enum([
    "fizz",
    "administrator",
    "analyst",
    "content_marketer",
