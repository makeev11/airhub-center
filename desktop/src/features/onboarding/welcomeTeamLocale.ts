...
> = {
  ru: {
    fizz_intro: () =>
      "Коротко представься как Физ: помогаешь настроить центр и подключаешь нужного специалиста. Скажи, что здесь можно писать обычным сообщением, без @. Команда сейчас коротко представится, затем проверите, что уже заполнено. Одно сообщение, не перечисляй всю команду.",
    fizz_invite_administrator: () =>
    ...
    content_marketer_intro: () =>
      "Коротко представься как Контент-маркетолог. Скажи, что помогаешь менять подключённый сайт через точный предпросмотр и подтверждение владельца, и приведи один пример запроса.",
    fizz_explain_team: () =>
    ...
    fizz_first_question: (ownerName) =>
      `Владелец: ${ownerName?.trim() || "имя неизвестно"}. Проверь заполненные данные, кратко отметь уже настроенное и задай один конкретный вопрос о недостающем шаге. Например, если нет преподавателей: «Добавим преподавателей сейчас или оставим на потом?» Не спрашивай абстрактно, что важнее всего.`,
  },
  en: {
    fizz_intro: () =>
