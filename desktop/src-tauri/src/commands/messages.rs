...
const AIRHOP_KICKOFF_STAGES: [&str; 5] = [
    "fizz_intro",
    "administrator_intro",
    "analyst_intro",
...
fn build_airhop_agent_task(
    ...
    if !AIRHOP_KICKOFF_STAGES.contains(&stage) {
    ...
    let tags = [
        ...
        vec![
            "airhop-kickoff-stage".to_string(),
            stage.to_string(),
        ],
    ]
    .into_iter()
    ...
    Ok(
        EventBuilder::new(
            Kind::Custom(buzz_core_pkg::kind::KIND_AIRHOP_AGENT_TASK as u16),
            instruction,
        )
        .tags(tags),
    )
...
pub async fn dispatch_airhop_agent_task(
    ...
    let channel_id = uuid::Uuid::parse_str(&channel_id)
        .map_err(|_| format!("invalid channel UUID: {channel_id}"))?;
    let builder = build_airhop_agent_task(
        channel_id,
        &agent_pubkey,
        &task_id,
        &stage,
