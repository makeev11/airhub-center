...
fn generated_passphrase_respects_word_count_and_separator() {
    ...
    // Use separators that cannot occur inside a word when validating the
    // number of generated words. The EFF list contains entries such as
    // "yo-yo", so splitting a hyphen-joined phrase is inherently ambiguous.
