...
manifest_version = re.search(r'(?m)^version = "([^"]+)"$', manifest_text)
if manifest_version is None:
    raise RuntimeError("desktop package version not found")
current_version = manifest_version.group(1)
manifest.write_text(
    manifest_text[:manifest_version.start(1)]
    + "9.8.7"
    + manifest_text[manifest_version.end(1):]
)
